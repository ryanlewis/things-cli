...
var viewFilters = map[string]string{
	"today":     "t.start = 1 AND t.startBucket = 0 AND t.startDate IS NOT NULL AND (t.status = 0 OR (t.status IN (2, 3) AND t.todayIndexReferenceDate = ?)) AND t.trashed = 0 AND COALESCE(p.trashed, 0) = 0 AND t.type = 0",
